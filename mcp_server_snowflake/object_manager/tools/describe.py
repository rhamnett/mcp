"""
Describe object tool for Snowflake MCP server.

This module provides the describe_object dynamic tool that can handle
describing of any Snowflake object type.
"""
...
async def describe_object(
    ...
    """
    Describe a Snowflake object of any supported type.

    This dynamic tool describes any type of Snowflake object based on the object_type
    parameter. It includes protection for system objects.

    Args:
        snowflake_service: Snowflake service instance
        object_type: Type of object to describe (database, schema, warehouse, role, database_role)
        name: Name of the object to describe
    ...
    Raises:
        SnowflakeException: If describe fails or parameters are invalid
    """
    try:
        ...
        if not name:
            raise SnowflakeException(
                tool="describe_object",
                message=f"'name' is required for describing {object_type}",
            )
        ...
        logger.info(f"Describing {object_type}: {name} (parent={parent_params})")

        # Call the manager's drop method
        result = await manager.describe(
            name=name,
        ...
        error_msg = f"Failed to describe {object_type}: {str(e)}"
        logger.error(error_msg)
        raise SnowflakeException(tool="describe_object", message=error_msg)


def describe_object_wrapper(snowflake_service: Any):
    """
    Create a wrapper for describe_object that pre-binds the snowflake_service parameter.
    ...
    async def wrapper(
        ...
        See describe_object function for full parameter documentation.
        """
        return await describe_object(
