...
class CoreObjectManager:
        ...
        **kwargs
        ...
        try:
            ...
            if self.object_type == 'warehouse':
                warehouse_bool_params = ['auto_resume', 'initially_suspended', 'enable_query_acceleration']
    ...
    async def list(
        ...
        """
        try:
            logger.info(f"Listing {self.object_type}s with filters: like={like}, starts_with={starts_with}, limit={limit}")
    ...
    async def drop(
        ...
        """
        try:
            logger.info(f"Dropping {self.object_type}: {name} (if_exists={if_exists}, cascade={cascade})")
            ...
            try:
                ...
                if if_exists:
                    logger.info(f"{self.object_type.capitalize()} '{name}' does not exist (if_exists=True)")
                    return {
                        'success': True,
                        'object_type': self.object_type,
                        'name': name,
